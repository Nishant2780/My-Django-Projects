...
def add_quotations_type(request):
    if request.method == 'POST':
        
        p_id = Products_Details.objects.get(id=request.POST.get('Product_id'))


        form = quotations_Form(request.POST, request.FILES)
        if form.is_valid():
            data = form.save(commit=False)
            profit = (int(request.POST.get('Product_price')) - int(p_id.Products_base_price)) * int(request.POST.get('Product_quantity'))
        ...
        form = quotations_Form()
    return render(request, 'manager_panel/add_quotations_type.html', {'form': form})

def quotations_list(request):
    types = quotations.objects.all()
    return render(request, 'manager_panel/quotations_list.html', {'types': types})